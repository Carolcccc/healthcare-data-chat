...
# Page configuration
st.set_page_config(
    page_title="Healthcare AI Agent (Gemini)",
    page_icon="🏥",
    layout="wide",
...
def initialize_agent(api_key):
    ...
    try:
        ...
        st.session_state.agent = create_agent_gemini(csv_path)
        st.session_state.api_key_set = True
        return True
    except Exception as e:
        st.error(f"Error loading dataset or initializing agent: {str(e)}")
        return False
...
if st.session_state.agent is None:
    ...
    This healthcare AI agent uses **Google Gemini Pro** to analyze healthcare data.
    
    **Features:**
    - 🤖 Cloud-based AI (Google Gemini)
    - 📊 Interactive data analysis
    - 💬 Natural language queries
    - 📈 Real-time insights
    - ☁️ Can be deployed to Streamlit Cloud
    
    **How to use:**
    1. Get a free API key from [Google AI Studio](https://makersuite.google.com/app/apikey)
    2. Enter your API key in the sidebar
    3. Click "Initialize Agent"
    4. Ask questions about the healthcare dataset
    
    **Note:** This version uses Gemini API instead of local Ollama, making it deployable to cloud platforms.
    """)
else:
    # Chat interface
...
st.markdown("""
    <div style='text-align: center; color: #666; padding: 1rem;'>
        <small>Healthcare AI Agent • Powered by Google Gemini Pro • Built with Streamlit & LangChain</small>
        <br>
